...
ax.set_ylabel(r'$ Integrated \ L_{bol}  [RGB \ pixel \ sum]$')

# plot total bol lum against used epsilon
ax.plot(eps_arr, lum_arr, 'b-')

# plot insets with saved images
d_eps = 0.5  # param to det size of inset in eps length-scales
d_lum = np.max(lum_arr)/7  # in lum length-scales

inset_start = ax.inset_axes([eps_arr[1] - d_eps/2 + d_eps/10, lum_arr[1] - d_lum/2 - d_lum, d_eps, d_lum], transform=ax.transData)
...
inset_dec = ax.inset_axes([eps_arr[mid_ind] - d_eps/2, lum_arr[mid_ind] - d_lum/2 - 1.2*d_lum, d_eps, d_lum], transform=ax.transData)
