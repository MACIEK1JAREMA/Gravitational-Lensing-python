# low mass companion in microlensing
...
# from the found L_{bol} extract the positions of peaks, plot these
# find the ratios of their heights and print to user
...
print('original transit data, contains these ratios: ' + '\n')
for i in range(int(np.floor(len(lum_maxima_r)/2))):
    if i % 2 == 0:
        print(lum_maxima_r[2*i]/lum_maxima_r[2*i + 1])
    else:
        print(lum_maxima_r[2*i+1]/lum_maxima_r[2*i])
...
print('lensed transit data, contains these ratios: ' + '\n')
for i in range(int(np.floor(len(lum_lensed_maxima_r)/2))):
    if i % 2 == 0:
        print(lum_lensed_maxima_r[2*i]/lum_lensed_maxima_r[2*i + 1])
    else:
        print(lum_lensed_maxima_r[2*i+1]/lum_lensed_maxima_r[2*i])
